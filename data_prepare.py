        ...
        if cv_train:
            ...
            if shuffle:
                if handle_large:
                    ...
                    ds = ds.shuffle(
                        buffer_size=int(len(labels) * (n_splits - 1) / n_splits)
                    )  # just memory is not enough ...
...
def _load_embeddings_only_for_fasttext_crawl_avg(path):
    ...
    with open(path, "r") as f:
        for i, line in enumerate(f):
            ...
            # print(line)
            hidden_dim = (
                len(line.split(" ")) - 2 if line.strip().split(" ")[-1] == "\n" else 1
            )  # for crawl, the '\n' is there to
            # print(hidden_dim)
            break
    ...
    def get_distribution_subgroups(self):
        ...
        for g in IDENTITY_COLUMNS:
            dstr[g] = self.get_distribution(
                self.df[self.df[g + "_in_train"].fillna(0.0) >= 0.5][TARGET_COLUMN]
            )  # could use continuous data, might be helpful so calculate belongs (fuzzy logic)
        return dstr
...
class BiasBenchmark:
    def __init__(self, train_df_id_na_dropped, threshold=0.5):
        ...
        self.validate_df = BiasBenchmark.copy_convert_dataframe_to_bool(
            train_df_id_na_dropped[IDENTITY_COLUMNS + [TARGET_COLUMN, TEXT_COLUMN]],
            threshold,
        )
    ...
    def calculate_benchmark(self, pred=None, validate_df=None, model_name=MODEL_NAME):
        ...
        validate_df = BiasBenchmark.copy_convert_dataframe_to_bool(
            validate_df[IDENTITY_COLUMNS + [TARGET_COLUMN, TEXT_COLUMN, model_name]],
            self.threshold,
        )
        (
            bias_metrics_df,
...
class EmbeddingHandler:
    ...
    def contraction_normalize(self):
        ...
        if not self._contraction_handled:
            ...
            self.df["treated_comment"] = self.df["lowered_comment"].apply(
                lambda x: clean_contractions(x, EmbeddingHandler.contraction_mapping)
            )
    ...
    def check_coverage(vocab, embeddings_index):
        ...
        print(
            "Found embeddings for {:.2%} of vocab".format(len(known_words) / len(vocab))
        )
        print(
    ...
    def prepare_tfrecord_data(
        ...
        set_trace()
        if (
            action is not None and action == lstm.CONVERT_DATA_Y_NOT_BINARY
        ):  # unpicker, change y
        ...
        if embedding:
            ...
            f = EMBEDDING_FILES[0]
            if (
                action is not None and action == lstm.CONVERT_ADDITIONAL_NONTOXIC_DATA
            ):  # unpicker, change y
    ...
    def data_prepare(self, action=None):
        ...
        if not self.do_emb_matrix_preparation:
            ...
            if action is not None:  # exist data, need to convert data
                ...
                if (
                    action == lstm.CONVERT_TRAIN_DATA
                    or action == lstm.CONVERT_ADDITIONAL_NONTOXIC_DATA
                ):
